...
import { IHorseStoreV2 } from "./interface/IHorseStoreV2.sol";
 ...
 */
contract HorseStoreYul is IHorseStoreV2, ERC721Enumerable {
    ...
    function feedHorse(uint256 horseId) external {
        assembly {
            let p := horseIdToFedTimeStamp.slot     // [horseId]

            mstore(0x00, horseId)                   // []           // Memory: [0x00: horseId]
            mstore(0x20, p)                         // []           // Memory: [0x00: horseId, 0x20: p]

            let slot := keccak256(0x00, 0x40)       // []           // Memory: [0x00: horseId, 0x20: p]
            sstore(slot, timestamp())               // []           // Memory: [0x00: horseId, 0x20: p]; Storage[slot] = block.timestamp
        }
    }
    ...
    function isHappyHorse(uint256 horseId) external view returns (bool) {
        assembly {
            let p := horseIdToFedTimeStamp.slot

            mstore(0x00, horseId)
            mstore(0x20, p)

            let slot := keccak256(0x00, 0x40)
            let lastFedTimeStamp := sload(slot)
            let cutoff := sub(timestamp(), HORSE_HAPPY_IF_FED_WITHIN)
            
            let isHappy := and(gt(lastFedTimeStamp, 0), gt(lastFedTimeStamp, cutoff))
